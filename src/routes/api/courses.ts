...
courses.post('/', (request, response) => {
    ...
    .then((course) => {
        ...
        response.status(501).send("cannot post courses")
    })
...
courses.get('/:id/batches', (req, res) => {   
    ...
    .then((course) => {
        ...
        res.status(501).send("cannot get batches by id")
    })
});
...
courses.get('/:courseId/batches/:batchId', (req, res) => {
    ...
    }).catch((err)=>{res.status(501).send("cannot get courses by id")})
});
